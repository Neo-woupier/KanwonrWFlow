    ...
    <div className="border border-zinc-700 rounded-md overflow-hidden bg-zinc-950">
      <Table>
        ...
        <TableBody className="divide-y divide-zinc-800/80">
          {filteredTasks.map((task) => (
            <TableRow key={task.id} className="hover:bg-zinc-900/50 transition-colors border-b border-zinc-800/80 last:border-b-0">
                ...
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-zinc-500 mt-1 flex items-center space-x-2">
                      <span>#{task.id}</span>
                      <span>•</span>
                      <span className="text-orange-400/80 font-medium">Deadline: {task.deadline}</span>
                    </div>
                  </div>
                </div>
              </TableCell>
      ...
      {filteredTasks.length === 0 && (
        <div className="p-12 flex flex-col items-center justify-center text-center border-t border-zinc-800">
          ...
          <p className="text-zinc-500 text-sm">Click &quot;New&quot; to create your first task.</p>
        </div>
      )}
    </div>
  );
